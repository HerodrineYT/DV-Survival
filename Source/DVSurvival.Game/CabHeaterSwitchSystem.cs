...
namespace DVSurvival.Mod
{
    ...
    {
        private const string GeneratedRootName = "DVSurvival_CabHeaterSwitch";
        ...
        public bool Ensure(TrainCar car)
        {
            ...
            if (UseSeasons()) return SeasonsCabHeating.Ensure(car);
            ...
            try
            {
                ...
                if (!TryCreateSwitch(interior.transform, profile, out created))
                {
                    ...
                    Debug.LogWarning("[DVSurvival] Cab heater switch could not be created for " +
                        car.carType + ".");
                    return false;
                }
            ...
            {
                RecordSetupFailure(interiorId);
                Debug.LogWarning("[DVSurvival] Cab heater switch setup failed for " + car.carType +
                    ": " + exception.Message);
                return false;
            }
        }
        ...
        {
            if (UseSeasons()) return SeasonsCabHeating.GetLevel(car);
            return IsSupported(car) ? GetLevel(GetCarId(car)) : 0f;
        }

        public float GetLevel(string carId)
        {
            if (UseSeasons()) return SeasonsCabHeating.GetLevel(carId);
        ...
        private bool UseSeasons()
        {
            if (!SeasonsCabHeating.Active) return false;
            // Restore any old generated geometry before Seasons binds it. Old
            // Survival save/network checkpoints must not overwrite its setting.
            if (bindings.Count > 0) Reset();
            return true;
        }
        ...
        private void HandleUserToggle(SwitchBinding binding, float level)
        {
            ...
            {
                Debug.LogWarning("[DVSurvival] Cab heater switch callback failed: " +
                    exception.Message);
            }
        }
        ...
        private sealed class SwitchBinding
        {
            ...
            public SwitchBinding(CabHeaterSwitchSystem owner, string carId, GameObject interior,
                ...
                MeshRenderer hiddenNativeRenderer, MeshFilter bodyFilter, Mesh originalBodyMesh)
            {
                this.owner = owner;
                CarId = carId;
                Interior = interior;
    ...
    internal sealed class CabHeaterControlNameHolder : ControlNameHolderBase
    {
        ...
        public override (string value, string unit) GetName()
        {
            ...
            if (level <= 0f) return (ModLocalization.Text("Отопитель: выключен", "Heater: off"), string.Empty);
            return (ModLocalization.Text("Отопитель: включён", "Heater: on"), string.Empty);
        }
    }
}
